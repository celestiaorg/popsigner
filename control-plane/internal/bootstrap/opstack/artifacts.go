...
// POPSigner manages all keys including Celestia signing.
func GenerateEnvExample(cfg *DeploymentConfig, addrs *ContractAddresses) string {
	// Determine Celestia network and RPC based on L1 chain
	celestiaNetwork := "mocha-4"
	celestiaRPC := "wss://mocha-4.celestia.io/websocket"
	if cfg.L1ChainID == 1 {
		celestiaNetwork = "celestia"
		celestiaRPC = "wss://celestia.io/websocket"
	}
	...
	popsignerCelestia := "https://celestia.popsigner.com"
...
# 
# Copy this file to .env and fill in the POPSIGNER_API_KEY value.
...
################################################################################
# POPSigner manages all signing keys in a secure enclave.
# Get your API key from https://dashboard.popsigner.com

# POPSigner RPC endpoint for L1 transaction signing
POPSIGNER_RPC_URL=%s

# POPSigner endpoint for Celestia blob signing
POPSIGNER_CELESTIA_ENDPOINT=%s

# Your POPSigner API key (REQUIRED)
POPSIGNER_API_KEY=<REQUIRED: get from dashboard.popsigner.com>

################################################################################
# ROLE ADDRESSES - Pre-configured from POPSigner
################################################################################

# Sequencer: Signs L2 blocks
SEQUENCER_ADDRESS=%s

# Batcher: Submits L2 batches to DA layer
BATCHER_ADDRESS=%s

# Proposer: Submits L2 state roots to L1
PROPOSER_ADDRESS=%s

# Celestia signer: Submits blobs to Celestia (managed by POPSigner)
CELESTIA_SIGNER_ADDRESS=%s

################################################################################
# DEPLOYED CONTRACT ADDRESSES
################################################################################

# DisputeGameFactory proxy address (for op-proposer)
DISPUTE_GAME_FACTORY_ADDRESS=%s

################################################################################
# CELESTIA DATA AVAILABILITY
################################################################################
# POPSigner manages Celestia signing - no local keyring needed!

# Your chain's Celestia namespace (auto-generated from chain ID)
CELESTIA_NAMESPACE=%s

# Celestia node RPC endpoint
# Testnet: wss://mocha-4.celestia.io/websocket
# Mainnet: wss://celestia.io/websocket or your own node
CELESTIA_RPC_URL=%s

# Celestia network (mocha-4 for testnet, celestia for mainnet)
CELESTIA_NETWORK=%s
`,
		cfg.ChainName,
		cfg.L1RPC,
		...
		cfg.BatcherAddress,
		cfg.ProposerAddress,
		cfg.BatcherAddress, // Celestia signer is typically the batcher
		disputeGameFactory,
		celestiaNamespace,
		celestiaRPC,
		celestiaNetwork,
